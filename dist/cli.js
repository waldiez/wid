...
function buildSignVerifyMessage(c) {
  ...
  if (c.DATA && c.DATA.length > 0) {
    if (!(0, import_node_fs.existsSync)(c.DATA)) {
...
function runSign(c) {
  ...
  if (c.OUT && c.OUT.length > 0) {
    (0, import_node_fs.writeFileSync)(c.OUT, out, "utf8");
  } else {
    console.log(out);
  }
  return 0;
...
function resolveWOtpSecret(raw) {
  if ((0, import_node_fs.existsSync)(raw)) {
    return (0, import_node_fs.readFileSync)(raw, "utf8").trim();
  }
  return raw.trim();
}
...
function sqlStatePath(c) {
  const dDir = c.D && c.D.length > 0 ? (0, import_node_path.resolve)(c.D) : (0, import_node_path.resolve)(".local/services");
  return (0, import_node_path.resolve)(dDir, "wid_state.sqlite");
}
...
function dataDir(c) {
  return c.D && c.D.length > 0 ? (0, import_node_path2.resolve)(c.D) : (0, import_node_path2.resolve)(".local/services");
}
...
function runCanonical(args) {
  ...
  (0, import_node_fs2.mkdirSync)(canonDataDir, { recursive: true });
  switch (c.A) {
    case "next":
